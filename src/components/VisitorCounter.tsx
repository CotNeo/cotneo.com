...
class VisitorService {
  ...
  private static readonly VISITOR_ID_KEY = 'visitorId';
  private static readonly CONSENT_KEY = 'consentGiven';
  private static readonly WELCOME_KEY = 'hasShownWelcome';

  static async fetchVisitorData(): Promise<ApiResponse> {
    try {
  ...
  static async registerVisitor(): Promise<void> {
    try {
      const response = await fetch(this.API_ENDPOINT, {
        method: 'POST',
        headers: {
...
const VisitorChart = ({ data, timeRange }: { data: VisitorData[]; timeRange: string }) => {
...
const ConsentBanner = ({ onAccept, onDecline }: { onAccept: () => void; onDecline: () => void }) => (
...
const VisitorCounter = () => {
  ...
  const fetchVisitorData = useCallback(async () => {
    try {
      ...
      const data = await VisitorService.fetchVisitorData();
      ...
      if (!VisitorService.getVisitorId() && data.visitorId) {
        VisitorService.setVisitorId(data.visitorId);
      }
    } catch (err) {
  ...
  const handleConsent = useCallback(async (accepted: boolean) => {
    try {
      ...
      VisitorService.setConsent(accepted);
  ...
  useEffect(() => {
    const checkConsent = () => {
      const consent = VisitorService.getConsent();
      if (consent === true) {
        fetchVisitorData();
        VisitorService.registerVisitor().catch(console.error);
      ...
      <div className="bg-gradient-to-br from-gray-800/50 to-gray-900/50 backdrop-blur-sm rounded-xl p-6 border border-gray-700/50 shadow-xl hover:shadow-2xl transition-all duration-500">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          <div className="bg-gray-800/30 rounded-lg p-4 border border-gray-700/50">
            ...
            <div className="text-3xl font-bold text-blue-400">
              {isLoading ? <LoadingSpinner /> : visitorCount}
            </div>
          </div>
        ...
        <div className="h-80 bg-gray-800/30 rounded-lg p-4 border border-gray-700/50">
          {isLoading ? (
            ...
            <ErrorDisplay error={error} onRetry={fetchVisitorData} />
          ) : (
            <VisitorChart data={visitorData} timeRange={timeRange} />
          )}
        </div>
